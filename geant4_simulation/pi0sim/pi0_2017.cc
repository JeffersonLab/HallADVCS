...
int main( int argc, char** argv)
{
 ...
 // if(1)//for visualization
  if( argc == 3 || argc == 4 )
    {
      dvcsGlobals::db = db;     
cout<<"    db   P o i n t e r    i n    t h e    i n i t i a l i z a t i o n    is    "<<db<<endl;
      int run_number = atoi(argv[2]);
     // int run_number = 12508; //testing visualization
  ...
  else
    {
      ...
      cout<<"./pi0_2017 pi0_run.mac run_number"<<endl;
      cout<<"where:"<<endl;
      cout<<"dvcs - executable"<<endl;
      cout<<"pi0_run.mac - a script which specifes number of events and few other things needed by GEANT4"<<endl;
  ...
  double targoff=0.0;
 // if(run_number==361)targoff=5;//0.75;//cm
 // if(run_number==362)targoff=5;//0.25;//cm
 // if(run_number==363)targoff=5;//1.0;//cm

  dvcsGlobals::target_offset = 0.;//cm
  ...
  dvcsHistManager *hist = new dvcsHistManager(obj_man);
...
// #endif
    ...
    { 
      ...
      G4UIExecutive * ui = new G4UIExecutive(argc,argv);
#ifdef G4VIS_USE
      UImanager->ApplyCommand("/control/execute pi0_vis.mac");
#endif
      ui->SessionStart();
      delete ui;
#endif
